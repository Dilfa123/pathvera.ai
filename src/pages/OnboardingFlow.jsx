...
export default function OnboardingFlow({ onCompleteOnboarding, isAuthenticated, onNavigateToAuth, onLogout }) {
  ...
  useEffect(() => {
    if (stepperActiveRef.current) {
      ...
      gsap.to(stepperActiveRef.current, { width: targetWidth, duration: 0.6, ease: 'power2.out' });
    }
  }, [currentStep]);

  const handleCardMouseEnter = (e) => {
    gsap.to(e.currentTarget, { scale: 1.03, borderColor: '#FFD166', duration: 0.2, overwrite: 'auto' });
  };

  const handleCardMouseLeave = (e) => {
    gsap.to(e.currentTarget, { scale: 1, borderColor: '', duration: 0.2, overwrite: 'auto' });
  };

  const handleCardClick = (e, callback) => {
    const target = e.currentTarget;
    const tl = gsap.timeline({ onComplete: callback });
    tl.to(target, { backgroundColor: 'rgba(255, 209, 102, 0.15)', duration: 0.15, ease: 'power1.out' })
      .to(target, { backgroundColor: '', duration: 0.25, ease: 'power1.inOut' });
  };

  const handleBtnMouseEnter = (e) => {
    if (e.currentTarget.disabled) return;
    gsap.to(e.currentTarget, { scale: 1.04, duration: 0.2, overwrite: 'auto' });
  };
  ...
  const handleBtnClick = (e, callback) => {
    ...
    const target = e.currentTarget;
    const tl = gsap.timeline({ onComplete: callback });
    tl.to(target, { scale: 0.97, duration: 0.1, ease: 'power1.out' })
      .to(target, { scale: 1.04, duration: 0.35, ease: 'back.out(1.7)' });
  };
  ...
  const handleRoleSelect = (role) => { setSelectedRole(role); setSelectedObjective(null); };
  const handleObjectiveSelect = (objId) => { setSelectedObjective(objId); };
  const toggleCuration = (key) => { setCurations(prev => ({ ...prev, [key]: !prev[key] })); };
  ...
  const handleNext = () => {
    if (currentStep < 4) { changeStep(currentStep + 1, false); }
    else { changeStep(currentStep, true); }
  };

  const handleBack = () => {
    if (currentStep > 1 || isFinalized) {
      if (isFinalized) { changeStep(4, false); }
      else { changeStep(currentStep - 1, false); }
    }
  };
  ...
  const resetFlow = () => {
    ...
    setCurations({ legacy: true, skills: true, network: false, compensation: false });
    changeStep(1, false);
  };
  ...
  return (
    <div className="onboarding-flow-container">
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .content-main {
          z-index: 1; width: 100%; max-width: 800px;
          display: flex; flex-direction: column; align-items: center;
          flex-grow: 1; justify-content: center; margin: 32px 0;
          perspective: 900px;
        }
        ...
        .logo-title {
          ...
          color: var(--text); letter-spacing: 0.12em; margin: 0;
        }
        .logo-title .logo-dot {
        ...
        .logo-subtitle {
          ...
          text-transform: uppercase; margin-top: 6px; margin-bottom: 0;
        }

        /* 4-step horizontal stepper */
        .stepper-container {
          display: flex; align-items: center; justify-content: space-between;
          width: 100%; max-width: 600px; margin-bottom: 48px; position: relative;
        }
        .stepper-line {
          position: absolute; top: 15px; left: 6%; right: 6%;
          height: 1px; background-color: var(--border); z-index: -1;
        }
        .stepper-line-active {
          position: absolute; top: 15px; left: 6%; height: 2px;
          background-color: var(--gold); z-index: 0;
        ...
        @keyframes leading-edge-pulse {
          0% { box-shadow: 0 0 4px rgba(255,209,102,0.6), 0 0 12px rgba(255,209,102,0.4); transform: translateY(-50%) scale(1); }
          50% { box-shadow: 0 0 12px rgba(255,209,102,1), 0 0 24px rgba(255,209,102,0.8); transform: translateY(-50%) scale(1.3); }
          100% { box-shadow: 0 0 4px rgba(255,209,102,0.6), 0 0 12px rgba(255,209,102,0.4); transform: translateY(-50%) scale(1); }
        }
        ...
        .step-bubble {
          ...
          color: var(--muted); margin-bottom: 12px; z-index: 1;
          transition: all 0.3s ease; letter-spacing: 0.05em;
        }
        .step-item.active .step-bubble {
          border-color: var(--gold); color: var(--gold);
          box-shadow: var(--glow-gold); transform: scale(1.05);
        }
        .step-item.completed .step-bubble {
          border-color: var(--gold); background-color: var(--gold);
          color: var(--bg); box-shadow: 0 0 12px rgba(255,209,102,0.3);
        }
        .step-label {
          font-family: monospace; font-size: 9px; font-weight: 700;
          color: var(--muted); letter-spacing: 0.4em;
        ...
        .step-item.active .step-label::after {
          ...
          height: 1px; background-color: var(--gold); border-radius: 2px;
        ...
        .step-subtitle {
          ...
          color: var(--muted); margin: 0 0 36px 0; text-align: center; line-height: 1.85;
        }

        /* Cards Row (Step 1) */
        .cards-row { display: flex; gap: 24px; justify-content: center; width: 100%; margin-bottom: 40px; }
        .identity-card {
          flex: 1; max-width: 300px; background-color: var(--card);
        ...
        .icon-circle {
          width: 56px; height: 56px; background-color: var(--gold-dim);
          border-radius: 50%; display: flex; align-items: center; justify-content: center;
          color: var(--gold); margin-bottom: 24px; transition: transform 300ms ease;
        }
        ...
        .card-desc { font-size: 13px; color: var(--muted); line-height: 1.65; margin: 0; }

        /* Step 2 (Objectives) */
        .objectives-grid {
          display: grid; grid-template-columns: repeat(2, 1fr);
          gap: 20px; width: 100%; max-width: 680px; margin-bottom: 40px;
        }
        .objective-card {
        ...
        /* Step 4 (Finalize) */
        .summary-card {
          width: 100%; max-width: 500px; background-color: var(--surface);
          border: 1px solid var(--border); border-radius: 16px;
          padding: 32px; box-sizing: border-box; margin-bottom: 40px;
        ...
        .summary-title {
          ...
          padding-bottom: 12px; margin: 0; text-align: left;
        }
        .summary-row {
          display: flex; justify-content: space-between; font-size: 13px;
          padding-bottom: 6px; border-bottom: 1px solid rgba(255,255,255,0.02);
        }
        .summary-key { color: var(--muted); font-family: monospace; font-size: 10px; letter-spacing: 0.1em; text-transform: uppercase; }
        .summary-value { font-weight: 400; color: var(--text); text-transform: capitalize; }
        .summary-curation-list { display: flex; flex-direction: column; gap: 6px; align-items: flex-end; }
        .curation-badge {
          font-size: 10px; background-color: var(--blue-dim);
          border: 1px solid rgba(74,158,255,0.15); color: var(--blue-accent);
          padding: 2px 8px; border-radius: 4px;
        }
        .success-panel {
          background-color: rgba(255,209,102,0.05);
          border: 1px solid rgba(255,209,102,0.15); border-radius: 8px;
        ...
        .control-btn {
          ...
          letter-spacing: 0.08em; padding: 14px 32px; cursor: pointer;
        ...
        .control-btn-next {
          background-color: var(--gold); color: var(--bg); border: 1px solid var(--gold);
          border-radius: 8px;
        }
        .control-btn-next:hover:not(:disabled) { box-shadow: var(--glow-gold); filter: brightness(1.05); }
        .control-btn-next:disabled {
          background-color: var(--border); border-color: var(--border);
        ...
        .footer-section {
          ...
          border-top: 1px solid var(--border); padding-top: 24px; margin-top: 40px; z-index: 1;
        }
      ...
      <div className="content-main">
        <div className="stepper-container">
          ...
          {steps.map((step) => (
            ...
            >
              <div className="step-bubble">{currentStep > step.id ? '✓' : step.id}</div>
          ...
          <>
            {renderedStep === 1 && (
              <div className="step-content-box" ref={stepContentRef}>
                ...
                <div className="cards-row">
                  <div className={`identity-card ${selectedRole === 'student' ? 'selected' : ''}`}
                    ...
                    <div className="icon-circle">
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <path d="M6 12.5v5a6 6 0 0 0 12 0v-5" /><path d="M21.5 12v6" />
                      </svg>
                    </div>
                  ...
                  <div className={`identity-card ${selectedRole === 'professional' ? 'selected' : ''}`}
              ...
              <div className="step-content-box" ref={stepContentRef}>
                ...
                <div className="objectives-grid">
                  {selectedRole && objectivesList[selectedRole].map((obj) => (
                    <div key={obj.id} className={`objective-card ${selectedObjective === obj.id ? 'selected' : ''}`}
              ...
              <div className="step-content-box" ref={stepContentRef}>
                ...
                <div className="curations-box">
                  {curationsList.map((item) => (
                    <div key={item.key} className={`curation-item ${curations[item.key] ? 'active' : ''}`}
              ...
              <div className="step-content-box" ref={stepContentRef}>
                ...
                <div className="summary-card">
                  ...
                  <div className="summary-row">
                    ...
                    <span className="summary-value" style={{ maxWidth: '240px', textAlign: 'right' }}>{getObjectiveTitle()}</span>
                  </div>
            ...
            <div className="controls-row">
              ...
              <button 
                ...
                disabled={(currentStep === 1 && !selectedRole) || (currentStep === 2 && !selectedObjective)}
          ...
          <div className="step-content-box" ref={stepContentRef}>
            <div className="summary-card" style={{ textAlign: 'center', alignItems: 'center', gap: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', width: '100%', marginTop: '12px' }}>
                <button className="control-btn control-btn-next" onClick={(e) => handleBtnClick(e, onCompleteOnboarding)} 
